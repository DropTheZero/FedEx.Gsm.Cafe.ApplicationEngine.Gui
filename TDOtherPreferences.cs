...
// Type: FedEx.Gsm.Cafe.ApplicationEngine.Gui.TDOtherPreferences
...
{
  public class TDOtherPreferences : OtherPreferences
  {
    private TDShipDefl _tdShipDefl;
    private IContainer components;

    public TDOtherPreferences() => this.InitializeComponent();

    public void InitOtherPrefs(TDShipDefl tdShipDefl, Shipment.CarrierType carrier)
    {
      this._tdShipDefl = tdShipDefl;
      this.InitOtherPrefs((ShipDefl) this._tdShipDefl, carrier);
    }

    public override void ScreenToObject() => base.ScreenToObject();

    private void TDOtherPreferences_Load(object sender, EventArgs e)
    {
      this.chkTrackingNbrOverwrite.Visible = false;
      if (this._carrier == Shipment.CarrierType.Express)
      {
        this.chk3DayFreight.Visible = false;
      ...
      if (this.DesignMode || !(GuiData.CurrentAccount.Address.CountryCode == "CA"))
        return;
      this.txtCurrency.Text = "USD";
    }

    protected override DataTable GetLabelFormatTable()
    {
      return Utility.GetDataTable(Utility.ListTypes.LabelFormat);
    }
    ...
    {
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (TDOtherPreferences));
      ...
      this.Name = nameof (TDOtherPreferences);
      this.Load += new EventHandler(this.TDOtherPreferences_Load);
